...
from app.crud import is_there_user, save_music, save_user, \
        is_user_uploaded_audio, select_audio
from pydub import AudioSegment
from io import BytesIO 


DOWNLOAD_LINK: str = "http://0.0.0.0:8000/record?id={audio_id}&user={user_id}"
...
async def save_as_mp3_and_convert_link(id: Annotated[str, Form()],
    ...
    try:
        ...
        raise HTTPException(status.HTTP_503_SERVICE_UNAVAILABLE, 
                            detail="integrity error in database")
...
async def get_audio(id: int,
    ...
    if not is_user_allowed_to_download:
        raise HTTPException(status.HTTP_403_FORBIDDEN,
                            detail="you are not allowed to download file") 
